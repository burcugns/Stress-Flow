...
function Sidebar({ currentUser }) {
  ...
  return (
    <div className="w-64 bg-gray-100 shadow-md p-4">
      ...
      <div className="mb-6">
        <button
          onClick={() => setShowStressHistory(showStressHistory ? false : true)}
          ...
          <div className="pl-2">
            {stressHistory.length > 0 ? (
              stressHistory.map((h) => (
                ...
                >
                  <span className="text-gray-600">{h.formattedDate}</span>
                  <span className="text-blue-700 font-semibold">{h.score}</span>
                </div>
              ))
            ) : (
      ...
      <div className="mb-6">
        <button
          onClick={() =>
            setStressTestSummary(showStressTestSummary ? false : true)
          }
          className="w-full flex justify-between items-center font-semibold mb-2 px-2 py-1 hover:bg-gray-200 rounded"
        >
      ...
      <div className="mb-6">
        <button
          onClick={() => setShowMoodHistory(showMoodHistory ? false : true)}
          className="w-full flex justify-between items-center font-semibold mb-2 px-2 py-1 hover:bg-gray-200 rounded"
        >
            ...
            {moodHistory.length > 0 ? (
              moodHistory.map((mood) => (
                <div
                  ...
                  className="border-b border-gray-200 py-2 flex justify-between text-sm"
                >
                  <span className="text-gray-600">{mood.formattedDate}</span>
                  <span className="text-green-600 font-semibold">
